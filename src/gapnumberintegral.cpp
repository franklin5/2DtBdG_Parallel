...
double cSeek_Gap_Number::compute(double delta, double mu, int i){
	...
	y1=pow(k1,j)*integrand_k(delta,mu, k1,i);
	y2=pow(k2,j)*integrand_k(delta,mu, k2,i);
	y3=pow(k3,j)*integrand_k(delta,mu, k3,i);
...
double cSeek_Gap_Number::integrand(double delta, double mu,
		double kx, double ky, int i){
	double result;
	double xi = kx*kx +ky*ky - mu;
	double temp = _h*_h+_v*_v*(kx*kx+ky*ky),
...
double cSeek_Gap_Number::integrand_k(double delta, double mu,
		double k, int i){
	double result;
	double xi = k*k  - mu;
